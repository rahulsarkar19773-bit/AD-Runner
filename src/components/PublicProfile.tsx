...
import { Link as LinkIcon, Heart, ArrowLeft } from 'lucide-react';
...
export default function PublicProfile({ userId, onBack }: PublicProfileProps) {
  ...
  const fetchUserData = async () => {
    ...
    try {
      ...
      if (userSnap.exists()) {
        setProfileUser(userSnap.data() as UserProfile);
      }
  ...
  return (
    <div className="pb-12 space-y-8 animate-in slide-in-from-right-4 duration-300">
      <div className="flex items-center -ml-2 mb-2">
        <button 
          onClick={onBack}
          className="p-2 rounded-full hover:bg-slate-200 transition-colors mr-2 text-slate-700"
        >
      ...
      <div className="flex flex-col items-center space-y-4">
        <div className="relative">
          <div className="w-28 h-28 rounded-full bg-gradient-to-tr from-green-400 via-blue-500 to-indigo-600 p-1">
            <div className="w-full h-full bg-blue-500 rounded-full flex items-center justify-center text-4xl font-bold text-white overflow-hidden bg-gradient-to-br from-blue-400 to-blue-600 shadow-inner">
              {profileUser.photoURL ? (
                <img src={profileUser.photoURL} alt={profileUser.name} className="w-full h-full rounded-full object-cover" />
              ) : (
        ...
        <h2 className="text-2xl font-bold text-slate-900 tracking-tight">{profileUser.name}</h2>
        <p className="text-slate-500">@{profileUser.name.toLowerCase().replace(/\s+/g, '')}</p>
      </div>
      ...
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 mx-auto max-w-sm">
        <div className="flex justify-between items-center text-center">
          <div className="flex-1">
            <p className="text-2xl font-bold text-[#1e1b4b]">{Number(profileUser.likesGiven) || 0}</p>
            <p className="text-xs text-slate-500 mt-1">Likes Given</p>
          </div>
          <div className="w-px h-10 bg-slate-200"></div>
          <div className="flex-1">
            <p className="text-2xl font-bold text-[#1e1b4b]">{Number(profileUser.followers) || 0}</p>
            <p className="text-xs text-slate-500 mt-1">Followers</p>
          </div>
          <div className="w-px h-10 bg-slate-200"></div>
          <div className="flex-1">
            <p className="text-2xl font-bold text-[#1e1b4b]">{Number(profileUser.following) || 0}</p>
      ...
      <div className="space-y-4">
        ...
        {links.length > 0 ? (
          ...
          <div className="text-center p-12 bg-white rounded-3xl border border-dashed border-slate-200">
            <p className="text-slate-500">This user hasn't published any links yet.</p>
          </div>
        )}
      </div>
    </div>
